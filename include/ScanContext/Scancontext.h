...
public: 
	...
	Eigen::MatrixXd makeNumbercontext(pcl::PointCloud<SCPointType> & _scan_down);

    Eigen::MatrixXd makeAvrHeightcontext(pcl::PointCloud<SCPointType>& _scan_down, Eigen::MatrixXd& _desc_hor, Eigen::MatrixXd& _desc_ver);
    ...
    void makeAndSaveCoviewContextAndKeys(Eigen::MatrixXd& _desc);
    std::pair<int, float> detectHierarchicalLoopClosureID(Eigen::MatrixXd sc); // int: nearest node index, float: relative yaw  

public:
	const double LIDAR_HEIGHT = 1.7;
    const int    PC_NUM_RING = 20; 
    const int    PC_NUM_SECTOR = 60; 
    // double PC_MAX_RADIUS = 50;
    double PC_MAX_RADIUS = 25; 
