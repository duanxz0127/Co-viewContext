...
MatrixXd SCManager::makeNumbercontext(pcl::PointCloud<SCPointType>& _scan_down)
{
    //// 20240320 dxz
    TicToc t_making_desc;
...
MatrixXd SCManager::makeAvrHeightcontext(pcl::PointCloud<SCPointType>& _scan_down, Eigen::MatrixXd& _desc_hor, Eigen::MatrixXd& _desc_ver)
{
    TicToc t_making_desc;
    ...
    // main
    const int NO_POINT = 0;
    MatrixXd desc_height = NO_POINT * MatrixXd::Ones(PC_NUM_RING, PC_NUM_SECTOR);
    MatrixXd desc_number = NO_POINT * MatrixXd::Ones(PC_NUM_RING, PC_NUM_SECTOR);

    SCPointType pt;
    float azim_angle, azim_range; // wihtin 2d plane
    int ring_idx, sctor_idx;
    int last_ring_idx = 0;
    int last_sctor_idx = 0;
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        pt.x = _scan_down.points[pt_idx].x;
        pt.y = _scan_down.points[pt_idx].y;
        ...
        // every bin record the average height of the points in the bin
        desc_height(ring_idx - 1, sctor_idx - 1) = desc_height(ring_idx - 1, sctor_idx - 1) + pt.z;
        desc_number(ring_idx - 1, sctor_idx - 1) = desc_height(ring_idx - 1, sctor_idx - 1) + 1;
    }

    // reset no points to zero (for cosine dist later)
    for (int row_idx = 0; row_idx < desc_height.rows(); row_idx++)
    {
        for (int col_idx = 0; col_idx < desc_height.cols(); col_idx++)
        {
            desc_height(row_idx, col_idx) = desc_height(row_idx, col_idx) / desc_number(row_idx, col_idx);
        }
    }
...
void SCManager::makeAndSaveCoviewContextAndKeys(Eigen::MatrixXd& _desc)
{
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(_desc);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(_desc);
    std::vector<float> polarcontext_invkey_vec = eig2stdvec(ringkey);

    polarcontexts_.push_back(_desc);
    polarcontext_invkeys_.push_back(ringkey);
    polarcontext_vkeys_.push_back(sectorkey);
    polarcontext_invkeys_mat_.push_back(polarcontext_invkey_vec);
}

std::pair<int, float> SCManager::detectHierarchicalLoopClosureID(Eigen::MatrixXd sc)
{
    int loop_id{ -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")

    auto curr_key = eig2stdvec(makeRingkeyFromScancontext(sc));
